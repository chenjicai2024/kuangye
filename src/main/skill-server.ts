...
/** 本地 HTTP 服务器认证 token，启动时随机生成，所有请求必须携带 */
...
export function startSkillServer(engineController: SkillEngineController): string {
  if (server) {
  ...
  server.on('error', (err: NodeJS.ErrnoException) => {
    if (err.code === 'EADDRINUSE' && server && !triedFallback) {
      triedFallback = true
      console.warn(
