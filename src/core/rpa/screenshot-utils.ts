...
import { bboxToCropBounds } from './coord-utils'
...
type CaptureDisplay = ScreenshotCache['display']

export type CaptureWechatWindowResult =
  | {
      success: true
      screenshotBase64: string
      nativeImage: Electron.NativeImage
  ...
  | { success: false; error: string; display?: CaptureDisplay }
...
export async function captureChatMainArea(appType: AppType): Promise<Electron.NativeImage | null> {
  try {
    ...
    const windowInfo = await getWindowInfo(appType)
...
/**
 * 自动检测主显示器的任务栏区域。
 * 通过比较 display.bounds（全屏）和 display.workArea（排除任务栏）的差值计算。
 * 返回逻辑像素的 ScreenRect，适用于 captureScreenRegion 截图。
 */
export function detectTaskbarRegion(): ScreenRect | null {
  ...
  if (workArea.y + workArea.height < bounds.y + bounds.height) {
    return {
      ...
      width: bounds.width,
      height: bounds.y + bounds.height - workArea.y - workArea.height
    }
  }
  // 顶部任务栏 / 菜单栏（macOS 常见）
  ...
  if (workArea.x + workArea.width < bounds.x + bounds.width) {
    return {
      ...
      y: bounds.y,
      width: bounds.x + bounds.width - workArea.x - workArea.width,
      height: bounds.height
    }
  }
  return null
