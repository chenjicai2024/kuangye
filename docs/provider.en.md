...
The Kuangye desktop client abstracts the "analyze a screenshot and generate a reply" chat capability into a Provider. As an external integrator you only need to supply a `manifest.json` and a bundle entry file; the app takes care of downloading and installing it, reading its config, passing in the chat screenshot, and consuming the events the Provider returns.
...
interface ProviderInput {
  ...
  // --- The following fields are optional Provider v1 extension fields ---
  // Unique identifier for this observation; Provider can use it for logging/tracing
  observationId?: string
  // Current conversation identity produced by the chat-history extractor
  ...
  // Formatted chat-history context that a Provider may fold into its system prompt
  chatHistoryContext?: string
  // Normalized list of messages visible in the current screenshot
  visibleMessages?: Array<{ direction: 'in' | 'out'; speaker: string; text: string }>
}
```

These extension fields are only passed when the chat-history feature is enabled and the current target application supports it. Provider v1 compatibility remains unchanged: external Providers may safely ignore these fields without affecting parsing of existing fields such as `screenshot`, `appType`, and `memoryCards`, or basic reply behavior.
...
```text
file:///path/to/kuangye-dev-desktop/resources/providers/volcengine-ark/manifest.json
```
