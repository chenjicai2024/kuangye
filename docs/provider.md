...
Kuangye 桌面端把“截图分析并生成回复”的聊天能力抽象为 Provider。外部技术使用者只需要提供一份 `manifest.json` 和一个 bundle 入口文件，应用会负责下载安装、读取配置、传入聊天截图，并消费 Provider 返回的事件。
...
interface ProviderInput {
  ...
  // --- 以下字段为 Provider v1 的可选扩展字段 ---
  // 本次观察的唯一标识，Provider 可用于日志/调试追踪
  ...
  // 已格式化的聊天历史上下文，Provider 可拼入 system prompt
  chatHistoryContext?: string
  // 当前截图中可见的消息列表（已归一化），供 Provider 参考
  visibleMessages?: Array<{ direction: 'in' | 'out'; speaker: string; text: string }>
}
```

以上扩展字段仅在应用启用聊天记录功能、且当前目标应用支持时才会传入。Provider v1 兼容保持不变：外部 Provider 可以安全地忽略这些字段，不影响 `screenshot`、`appType`、`memoryCards` 等既有字段的解析与基础回复逻辑。
